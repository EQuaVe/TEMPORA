...
static struct option long_options[] = {{"asynchronous-events", no_argument, 0, 0},
                                       {"check-syntax", required_argument, 0, 'c'},
...
static char * const options = (char *)"c:d:hn:o:ptj";
...
void usage(char * progname)
{
  ...
  std::cerr << "   -c automaton_type      syntax check" << std::endl;
  std::cerr << "        ta                timed automaton" << std::endl;
  std::cerr << "        gta               generalized timed automaton" << std::endl;
...
static bool check_ta_syntax = false;
static bool check_gta_syntax = false;
...
int parse_command_line(int argc, char * argv[])
{
  while (true) {
    ...
    if (c == ':')
      ...
      switch (c) {
      case 'c':
        if (strcmp(optarg, "ta") == 0)
          check_ta_syntax = true;
        else if (strcmp(optarg, "gta") == 0)
          check_gta_syntax = true;
        else
          throw std::invalid_argument("Invlaid option for check-syntax");
        break;
      case 'd':
        delimiter = optarg;
        break;
...
/*!
 \brief Check generalized timed automaton syntax from a declaration
 \param sysdecl : system declaration
 \post error and warnings have been reported to std::cerr
*/
void do_syntax_check_gta(tchecker::parsing::system_declaration_t const & sysdecl)
{
  if (tchecker::tck_syntax::syntax_check_gta(std::cerr, sysdecl))
    std::cout << "Syntax OK" << std::endl;
}
