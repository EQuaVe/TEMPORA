...
void usage(char * progname)
{
  ...
  std::cerr << "          gtacovreach   reachability algorithm with covering over the gta zone graph" << std::endl;
...
enum algorithm_t {
  ...
  ALGO_GTACOVREACH, /*!< Covering reachability algorithm for generalized timed automata */
  ...
  while (true) {
    ...
    if (c == ':')
      ...
      case 'a':
        ...
        else if (strcmp(optarg, "gtacovreach") == 0)
          algorithm = ALGO_GTACOVREACH;
        else
...
/*!
 \brief Perform covering reachability analysis over the GTA zone graph
 \param sysdecl : system declaration
 \post statistics on covering reachability analysis of command-line specified
 labels in the system declared by sysdecl have been output to standard output.
...
*/
void gta_covreach(tchecker::parsing::system_declaration_t const & sysdecl)
{
  tchecker::algorithms::covreach::stats_t stats;
  std::shared_ptr<tchecker::tck_reach::gtacovreach::graph_t> graph;

  if (certificate == CERTIFICATE_SYMBOLIC || certificate == CERTIFICATE_CONCRETE) {
    throw std::runtime_error("symbolic or concrete certificate generation is not available for gta covreach algorithm");
  }
  ...
  std::tie(stats, graph) =
      tchecker::tck_reach::gtacovreach::run(sysdecl, labels, search_order, covering, block_size, table_size);
  ...
  // Certificate
  if (certificate == CERTIFICATE_GRAPH) {
    tchecker::tck_reach::gtacovreach::dot_output(*os, *graph, sysdecl.name());
  }
}
...
int main(int argc, char * argv[])
{
  try {
    ...
    case ALGO_COVREACH:
      covreach(*sysdecl);
      break;
    case ALGO_GTACOVREACH:
      gta_covreach(*sysdecl);
      break;
    default:
