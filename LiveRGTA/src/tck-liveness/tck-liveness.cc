...
void usage(char * progname)
{
  ...
  std::cerr << "          gtascc     Modified Couvreur's SCC-decomposition-based algorithm for GTA liveness" << std::endl;
  std::cerr << "                     search an accepting cycle that visits all labels and can be taken infinitely often"
            << std::endl;
...
enum algorithm_t {
  ...
  ALGO_GTASCC,  /*!< GTA SCC decomposition algorithm */
  ...
  while (true) {
    ...
    if (c == ':')
      ...
      case 'a':
        ...
        else if (strcmp(optarg, "gtascc") == 0)
          algorithm = ALGO_GTASCC;
        else
...
/*!
 \brief Run GTA SCC algorithm
 \param sysdecl : system declaration
 \post statistics on accepting run w.r.t. command-line specified labels in
 the system declared by sysdecl have been output to standard output.
 A certificate has been output if required.
*/
void gtascc(tchecker::parsing::system_declaration_t const & sysdecl)
{
  if (certificate != CERTIFICATE_NONE && certificate != CERTIFICATE_GRAPH) {
    throw std::runtime_error("*** only graph certificate supported for gta liveness");
  }

  auto && [stats, graph] = tchecker::tck_liveness::zg_gtascc::run(sysdecl, labels, block_size, table_size);
  ...
  // certificate
  if (certificate == CERTIFICATE_GRAPH) {
    tchecker::tck_liveness::zg_gtascc::dot_output(*os, *graph, sysdecl.name());
  }
}
