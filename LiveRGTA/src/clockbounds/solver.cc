...
class gmap_updater_t : tchecker::gta::typed_action_visitor_t {
public:
  gmap_updater_t(tchecker::clockbounds::gta::gsim_map_t & gmap) : _gmap(gmap) {}

  void update(tchecker::gta::compiled_gta_program_t const & gta_prog)
  {
    for (tchecker::gta::compiled_gta_program_t::iterator_t back_iter = gta_prog.backward_begin();
         back_iter != gta_prog.backward_end(); back_iter = gta_prog.prev(back_iter)) {
      if (back_iter.first != nullptr) {
        tchecker::clock_constraint_container_t const & cc = *(back_iter.first->get_value());

        for (tchecker::clock_constraint_t const & c : cc) {
          tchecker::dbm::extended::db_t const bound{c.comparator(), c.value()};
          if (c.id1() == tchecker::REFCLOCK_ID) {
            _gmap.add_zero_minus_x_bound(c.id2(), bound);
          }
          else if (c.id2() == tchecker::REFCLOCK_ID) {
            _gmap.add_x_minus_zero_bound(c.id1(), bound);
          }
          else {
            _gmap.add_diagonal_bound(c.id1(), c.id2(), bound);
          }
        }
      }

      else if (back_iter.second != nullptr) {
        tchecker::gta::typed_actions_t const & actions = *(back_iter.second->get_value());
        for (auto back_iter = actions.rbegin(); back_iter != actions.rend(); back_iter++) {
          (*back_iter)->visit(*this);
        }
      }
    }
  }
...
private:
  ...
  {
    tchecker::clock_id_t const num_clocks = _gmap.num_clks();
    tchecker::clock_id_t const release_clk = action.typed_clk_var()->clk_id();

    _gmap.clear_x_minus_zero_bounds(release_clk);
    _gmap.clear_zero_minus_x_bounds(release_clk);
    _gmap.clear_diagonal_bounds(release_clk, release_clk);

    for (tchecker::clock_id_t x = 0; x < num_clocks; x++) {
      if (x == release_clk) {
        continue;
      }

      tchecker::clockbounds::gta::bounds_collection_t const & x_minus_r_bounds = _gmap.diagonal_bounds(x, release_clk);
      for (tchecker::dbm::extended::db_t const & bound : x_minus_r_bounds) {
        _gmap.add_x_minus_zero_bound(x, bound);
      }
      _gmap.clear_diagonal_bounds(x, release_clk);

      tchecker::clockbounds::gta::bounds_collection_t const & r_minus_x_bounds = _gmap.diagonal_bounds(release_clk, x);
      for (tchecker::dbm::extended::db_t const & bound : r_minus_x_bounds) {
        _gmap.add_zero_minus_x_bound(x, bound);
      }
      _gmap.clear_diagonal_bounds(release_clk, x);
    }
  }
  ...
  {
    tchecker::clock_id_t const num_clocks = _gmap.num_clks();
    tchecker::clock_id_t const reset_clk = action.typed_clk_var()->clk_id();

    _gmap.clear_x_minus_zero_bounds(reset_clk);
    _gmap.clear_zero_minus_x_bounds(reset_clk);
    ...
    for (tchecker::clock_id_t x = 0; x < num_clocks; x++) {
      ...
      tchecker::clockbounds::gta::bounds_collection_t const & x_minus_r_bounds = _gmap.diagonal_bounds(x, reset_clk);
      for (tchecker::dbm::extended::db_t const & bound : x_minus_r_bounds) {
        _gmap.add_x_minus_zero_bound(x, bound);
      }
      _gmap.clear_diagonal_bounds(x, reset_clk);

      tchecker::clockbounds::gta::bounds_collection_t const & r_minus_x_bounds = _gmap.diagonal_bounds(reset_clk, x);
      for (tchecker::dbm::extended::db_t const & bound : r_minus_x_bounds) {
        _gmap.add_zero_minus_x_bound(x, bound);
      }
      _gmap.clear_diagonal_bounds(reset_clk, x);
    }
  }

  virtual void visit(tchecker::gta::typed_action_rename_prophecy_t const & action)
  {
    tchecker::clock_id_t const lclk = action.typed_lhs_clk_var()->clk_id();
    tchecker::clock_id_t const rclk = action.typed_rhs_clk_var()->clk_id();
    tchecker::clock_id_t const num_clks = _gmap.num_clks();

    if (lclk == rclk) {
      return;
    }

    _gmap.add_x_minus_zero_bounds_from(rclk, _gmap.x_minus_zero_bounds(lclk));
    _gmap.clear_x_minus_zero_bounds(lclk);

    _gmap.add_zero_minus_x_bounds_from(rclk, _gmap.zero_minus_x_bounds(lclk));
    _gmap.clear_zero_minus_x_bounds(lclk);

    _gmap.clear_diagonal_bounds(lclk, rclk);
    _gmap.clear_diagonal_bounds(rclk, lclk);
    ...
    for (tchecker::clock_id_t x = 0; x < num_clks; x++) {
      ...
      tchecker::clockbounds::gta::bounds_collection_t const & x_minus_lclk_bounds = _gmap.diagonal_bounds(x, lclk);
      for (tchecker::dbm::extended::db_t const & bound : x_minus_lclk_bounds) {
        _gmap.add_diagonal_bound(x, rclk, bound);
      }
      _gmap.clear_diagonal_bounds(x, lclk);

      tchecker::clockbounds::gta::bounds_collection_t const & lclk_minus_x_bounds = _gmap.diagonal_bounds(lclk, x);
      for (tchecker::dbm::extended::db_t const & bound : lclk_minus_x_bounds) {
        _gmap.add_diagonal_bound(rclk, x, bound);
      }
      _gmap.clear_diagonal_bounds(lclk, x);
    }
  }
  ...
  tchecker::clockbounds::gta::gsim_map_t & _gmap;
};

void compute_fixed_point(tchecker::clockbounds::gta::loc_gmap_t & loc_gmap, tchecker::gta::system_t const & system)
{
  tchecker::loc_id_t const num_locs = system.locations_count();

  std::unordered_set<tchecker::loc_id_t> locs_to_visit;

  for (tchecker::loc_id_t l = 0; l < num_locs; l++) {
    locs_to_visit.insert(l);
  }

  while (!locs_to_visit.empty()) {
    auto const & it = locs_to_visit.begin();
    tchecker::clock_id_t const tgt_loc = *it;

    locs_to_visit.erase(it);

    for (tchecker::system::edge_const_shared_ptr_t const & e : system.incoming_edges(tgt_loc)) {
      tchecker::edge_id_t const e_id = e->id();
      tchecker::loc_id_t const src = e->src();

      if (system.is_edge_unsatisfiable(e_id)) {
        continue;
      }

      tchecker::gta::compiled_gta_program_t const & compiled_gta_prog = system.compiled_gta_program(e_id);

      tchecker::clockbounds::gta::gsim_map_t tgt_gmap_copy(loc_gmap[tgt_loc]);
      tchecker::clockbounds::gta::details::gmap_updater_t updater(tgt_gmap_copy);

      updater.update(compiled_gta_prog);

      if (loc_gmap[src].add_bounds_from(tgt_gmap_copy)) {
        locs_to_visit.insert(src);
      }
    }
  }
}
...
std::ostream & output(std::ostream & os, loc_gmap_t const & loc_gmap, tchecker::clock_variables_t const & vars)
{

  for (tchecker::loc_id_t l = 0; l < loc_gmap.size(); l++) {
    std::cout << "Location " << l << ": ";
    loc_gmap[l].output(os, vars);
  }

  return os;
}

tchecker::clockbounds::gta::loc_gmap_t compute_gmap(tchecker::gta::system_t const & system)
{
  tchecker::loc_id_t const num_locs = system.locations_count();
  tchecker::clock_id_t const num_clks = system.clocks_count(tchecker::VK_FLATTENED);

  tchecker::clockbounds::gta::gsim_map_t init_bounds(num_clks);
  tchecker::clockbounds::gta::loc_gmap_t loc_gmap = loc_gmap_t(num_locs, init_bounds);

  tchecker::clockbounds::gta::details::compute_fixed_point(loc_gmap, system);

  for (tchecker::clock_id_t x = 0; x < num_clks; x++) {
    if (system.is_prophecy_clock(x)) {
      for (tchecker::loc_id_t l = 0; l < num_locs; l++) {
        loc_gmap[l].add_x_minus_zero_bound(x, tchecker::dbm::extended::LE_ZERO);
      }
    }
  }
  return loc_gmap;
