/*
 * This file is a part of the TChecker project.
 *
 * See files AUTHORS and LICENSE for copyright details.
 *
 */

#include "tchecker/ta/transition.hh"
#include "tchecker/basictypes.hh"
#if BOOST_VERSION <= 106600
#include <boost/functional/hash.hpp>
#else
#include <boost/container_hash/hash.hpp>
#endif

#include "tchecker/gta/transition.hh"

namespace tchecker {

namespace gta {

/* transition_t */

transition_t::transition_t(tchecker::sync_id_t sync_id, tchecker::vedge_sptr_t const & vedge)
    : tchecker::ta::transition_t(sync_id, vedge)
{
  assert(sync_id == tchecker::NO_SYNC);
}

transition_t::transition_t(tchecker::gta::transition_t const & t, tchecker::sync_id_t sync_id,
                           tchecker::vedge_sptr_t const & vedge)
    : tchecker::ta::transition_t(t, sync_id, vedge)
{
  assert(sync_id == tchecker::NO_SYNC);
}

bool operator==(tchecker::gta::transition_t const & t1, tchecker::gta::transition_t const & t2)
{
  return (tchecker::ta::operator==(t1, t2));
}

bool shared_equal_to(tchecker::gta::transition_t const & t1, tchecker::gta::transition_t const & t2)
{
  return tchecker::ta::shared_equal_to(t1, t2);
}

std::size_t hash_value(tchecker::gta::transition_t const & t) { return tchecker::ta::hash_value(t); }

std::size_t shared_hash_value(tchecker::gta::transition_t const & t) { return tchecker::ta::shared_hash_value(t); }

int lexical_cmp(tchecker::gta::transition_t const & t1, tchecker::gta::transition_t const & t2)
{
  return tchecker::ta::lexical_cmp(t1, t2);
}

} // end of namespace gta

} // end of namespace tchecker
