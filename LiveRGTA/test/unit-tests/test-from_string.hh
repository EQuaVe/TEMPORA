...
TEST_CASE("Parsing GTA Program", "[parse_gta_program]")
{
  std::string gta_program = "x == 0; [x], [y], z = a; y >= -12 && x - y <= -inf; ;";

  std::shared_ptr<tchecker::gta::gta_program_t> parsed_program;
  parsed_program = tchecker::test::parse_gta(gta_program);
  REQUIRE(parsed_program != nullptr);

  std::string parsed_program_as_string = parsed_program->to_string();
  REQUIRE(parsed_program_as_string == "x==0; [x],[y],z=a; x-y<=-inf && y>=-12; ; ");
