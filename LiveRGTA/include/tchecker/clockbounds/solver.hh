...
/*!
 * \brief Output the location to bounds map
 * \param os : Output stream
 * \param loc_gmap : Location to bounds map
 * \vars : Clock variables of the system
 * \return os after outputting the bounds to it
 */
std::ostream & output(std::ostream & os, loc_gmap_t const & loc_gmap, tchecker::clock_variables_t const & vars);

/*!
 * \brief Compute location to gsim bounds
 * \param system : GTA system
 * \return : The fixed point solution for gsim bounds along with x <= 0 bounds for all prophecy clocks x
 */
