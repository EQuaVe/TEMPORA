...
edge:P${p}:idle:acq:take${LEFT}{provided: fork${LEFT} == 0 : do: fork${LEFT} = 1; x${p}=0}
edge:P${p}:acq:idle:release${LEFT}{provided: x${p}>=${TIMEOUT} : do : fork${LEFT}=0}
edge:P${p}:acq:eat:take${RIGHT}{provided: fork${RIGHT} == 0 && x${p}<=${TIMEOUT} : do: x${p}=0; fork${RIGHT} = 1}
edge:P${p}:eat:rel:release${RIGHT}{provided: x${p}>=${EAT} : do: x${p}=0; fork${RIGHT} = 0}
edge:P${p}:rel:idle:release${LEFT}{do: fork${LEFT} = 0}
"
done
